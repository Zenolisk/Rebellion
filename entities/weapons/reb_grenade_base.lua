...
SWEP.Primary.ThrowForce			= 0
SWEP.Primary.ClipSize			= 1
SWEP.Primary.Automatic			= false
SWEP.Primary.ThrowEnt	        = ""
...
function SWEP:Think()
	if ((self.Weapon.nextThrow or 0) >= CurTime()) then
		return
	end
	...
	if self.Owner:KeyReleased(IN_ATTACK) then
		...
		self.Weapon.nextThrow = CurTime() + 1
		self.Weapon:EmitSound("reb/weapons/swing"..math.random(1,3)..".wav")
		self:Grenade(self.Primary.ThrowForce, self.Primary.ThrowEnt)
	elseif self.Owner:KeyReleased(IN_ATTACK2) then
		self:SendWeaponAnim(ACT_VM_THROW)
		self.Owner:SetAnimation(PLAYER_ATTACK1)
		self.Weapon.nextThrow = CurTime() + 1
		self.Weapon:EmitSound("reb/weapons/swing"..math.random(1,3)..".wav")
		self:Grenade(self.Primary.ThrowForce/2, self.Primary.ThrowEnt)
	end
...
function SWEP:Grenade(force, ent)
	if (SERVER) then
		...
		velocity = velocity + ( VectorRand() * 15 ) -- a random element
